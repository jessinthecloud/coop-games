...
dump('parent model: ',$model);
    ...
    public function where(
        ...
        if ( is_string( $value ) ) {
            if ( $operator === 'like' ) {
                ...
                $where->push(
                    ( $where->count() ? $boolean . ' ' : '' ) . $key . ' ' . $operator . ' "' . $value . '"'
                );
    ...
    public function whereIn(
        ...
        $where->push(
            ( $where->count() ? $boolean . ' ' : '' ) . $key . ' ' . $operator . ' ' . $prefix . $valuesString . $suffix
        );
    ...
    public function whereBetween(
        ...
        $this->whereNested( function ( $query ) use (
            ...
            $query->where( $key, '>' . $operator, $first )->where(
                $key,
                '<' . $operator,
                $second
            );
    ...
    public function whereNotBetween(
        ...
        $this->whereNested( function ( $query ) use (
            ...
            $operator = ( $withBoundaries ? '=' : '' );
            $query->where( $key, '<' . $operator, $first )->orWhere(
                $key,
                '>' . $operator,
                $second
            );
    ...
    protected function setEndpoint( $model ) : void
    {
        ...
        if ( $this->endpoint === null ) {
            $message = 'Construction-Parameter of Builder must be a string or a Class which extends ' . $neededNamespace . '\\Model. ' . ucfirst(
                    gettype( $model )
                ) . ' given.';
            throw new InvalidArgumentException( $message );
        }
     ...
     * @throws MissingEndpointException
     */
    protected function fetchApi( bool $count = false )
    {
        if ( !$this->endpoint ) {
     ...
     * @return string
     */
    protected function getEndpoint( bool $count = false ) : string
    {
